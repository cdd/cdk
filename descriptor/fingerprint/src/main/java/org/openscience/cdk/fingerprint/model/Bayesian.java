...
public class Bayesian
{
	...
	public Bayesian(int classType,int folding)
	{
		...
		boolean bad=false;
		if (folding>0) for (int f=folding;f>0;f=f>>1) if ((f&1)==1 && f!=1) {bad=true; break;}
		if (folding<0 || bad) throw new ArithmeticException("Fingerprint folding "+folding+" invalid: must be 0 or power of 2.");
	}
	 ...
	 */
	public int getFolding() {return folding;}
	 ...
	 */
	public int getTrainingActives() {return trainingActives;}
	...
	{
		if (title.indexOf("\n")>=0 || title.indexOf("\t")>=0) throw new IllegalArgumentException("Comments cannot contain newlines or tabs.");
		noteTitle=title;
	}
	 ...
	 */
	public String getNoteOrigin() {return noteOrigin;}
	 ...
	 */
	public void setNoteOrigin(String origin) {noteOrigin=origin;}
	...
	{
		if (comments!=null) for (String comment : comments) if (comment.indexOf("\n")>=0 || comment.indexOf("\t")>=0) 
	...
	public String serialise()
	{
		...
		if (noteComments!=null) for (String comment : noteComments) buff.append("note:comment="+comment+"\n");
	...
	public static Bayesian deserialise(BufferedReader rdr) throws IOException
	{
		...
		if (line==null || !line.startsWith("Bayesian!(") || !line.endsWith(")")) throw new IOException("Not a serialised Bayesian model.");
		...
		int classType=bits[0].equals("ECFP0") ? CircularFingerprinter.CLASS_ECFP0 :
					  ...
					  bits[0].equals("FCFP0") ? CircularFingerprinter.CLASS_FCFP0 :
					  bits[0].equals("FCFP2") ? CircularFingerprinter.CLASS_FCFP2 :
					  bits[0].equals("FCFP4") ? CircularFingerprinter.CLASS_FCFP4 :
					  bits[0].equals("FCFP6") ? CircularFingerprinter.CLASS_FCFP6 : 0;
		if (classType==0) throw new IOException("Unknown fingerprint type: "+bits[0]);

		int folding=Integer.valueOf(bits[1]);
		if (folding>0) for (int f=folding;f>0;f=f>>1) if ((f&1)==1 && f!=1) {folding=-1; break;}
		if (folding<0) throw new IOException("Fingerprint folding "+bits[1]+" invalid: must be 0 or power of 2.");
		...
		while (true)
		{
			...
			{
				try {model.trainingSize=Integer.valueOf(line.substring(14));}
				catch (NumberFormatException ex) {throw new IOException("Invalid training info line: "+line);}
			}
			else if (line.startsWith("training:actives="))
			{
				try {model.trainingActives=Integer.valueOf(line.substring(17));}
				catch (NumberFormatException ex) {throw new IOException("Invalid training info line: "+line);}
			}
			else if (line.startsWith("roc:auc=")) 
			{
				try {model.rocAUC=Double.valueOf(line.substring(8));}
				catch (NumberFormatException ex) {throw new IOException("Invalid AUC line: "+line);}
			}
			...
			else if (line.startsWith("roc:x="))
			{
				...
				{
					try {model.rocX[n]=Float.valueOf(nums[n]);}
					catch (NumberFormatException ex) {throw new IOException("Invalid ROC X coordinates, number="+nums[n]);}
				}
			}
			else if (line.startsWith("roc:y="))
			{
				...
				for (int n=0;n<nums.length;n++) 
				{
					try {model.rocY[n]=Float.valueOf(nums[n]);}
					catch (NumberFormatException ex) {throw new IOException("Invalid ROC Y coordinates, number="+nums[n]);}
				}
			}
	...
	private void validateNfold(int nsegs)
	{
		...
		estimates=new double[sz];
		for (int n=0;n<sz;n++) estimates[order[n]]=estimatePartial(order,n,segContribs[n%nsegs]);
		calculateROC();
	}
	...
	private Map<Integer,Double> buildPartial(final int order[],int seg,int div)
	{
		...
		for (int n=0;n<sz;n++) if (n%div!=seg) 
		{
	...
	private void calculateROC()
	{
		...
		for (int n=0;n<tsz;n++)
		{
			...
			for (;ipos<sz;ipos++)
			{
				...
				if (activity.get(idx[ipos])) posTrue++; else posFalse++;
			}
    ...
    private double[] resize(final double[] arr,int sz) 
    	{double[] ret=new double[sz]; for (int n=(arr==null ? 0 : Math.min(sz,arr.length))-1;n>=0;n--) ret[n]=arr[n]; return ret;}
    private float[] resize(final float[] arr,int sz) 
    	{float[] ret=new float[sz]; for (int n=(arr==null ? 0 : Math.min(sz,arr.length))-1;n>=0;n--) ret[n]=arr[n]; return ret;}
    private double[] reverse(double[] arr)
    	{double[] ret=new double[arr.length]; for (int i=0,j=arr.length-1;j>=0;i++,j--) ret[j]=arr[i]; return ret;}
    private float[] reverse(float[] arr)
    	{float[] ret=new float[arr.length]; for (int i=0,j=arr.length-1;j>=0;i++,j--) ret[j]=arr[i]; return ret;}
}
